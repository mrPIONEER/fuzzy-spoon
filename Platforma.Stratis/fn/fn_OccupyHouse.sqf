...
// Телепортирует юниты в случайные окна зданий на расстоянии
// Направляет юнитов в правильном направлении и приказывает им встать или присесть на крышу
// Юниты будут заполнять здание только на столько позиций, сколько есть окон
// Несколько зданий могут быть заполнены либо равномерно, либо до предела каждого последовательно
// Usage : Call, execVM
// Params: 1. Массив, используется здание (здания), ближайшее к этой позиции
//         2. Массив объектов, подразделений, которые будут гарнизировать здание (ы)
//  (opt.) 3. Скаляр, радиус, в котором нужно заполнить здание (а), -1 только для ближайшего здания, (по умолчанию: -1)
//  (opt.) 4. Boolean, true для установки юнитов на крыше, false только для внутренней части (по умолчанию: false)
//  (opt.) 5. Boolean, true, чтобы равномерно заполнить все здания в радиусе, false по одному (по умолчанию: false)
//  (opt.) 6. Boolean, true для заполнения сверху здания, внизу (по умолчанию: false)
//  (opt.) 7. Boolean, true, чтобы приказать юнитам ИИ перемещаться в позицию вместо телепортации (по умолчанию: false)
// Возврат: массив объектов, единиц, которые не были гарнизонными

// [getPosATL this, units group this] call PL_fnc_OccupyHouse; бойцы занимают здание в окнах
