...
nul=[this,"town"] spawn PL_fnc_UPS // стандарт
nul=[this,"town","nofollow"] spawn PL_fnc_UPS // охрана зоны
nul=[this,"post1","nofollow","nomove"] spawn PL_fnc_UPS   // Для стоящих на посту

//
//  Optional parameters: 
//    random        = Поместите юнит в случайное начальное положение.
//    randomdn      = Используйте только случайные позиции на уровне земли.
//    randomup      = Используйте только случайные позиции на верхних строительных постах.
//    min:n/max:n   = Создайте случайное число (от минимального до максимального) «клонов».
//    init:string   = Строка инициации клонированных юнитов. (НЕ в Arma3!)
//    prefix:string = Имена клонированных подразделений будут начинаться с этого префикса.
//    nomove        = Юнит будет оставаться на стартовой позиции, пока не будет замечен враг.
//    nofollow      = Подразделение будет следовать за врагом только в зоне маркера.
//    delete:n      = Удалите мертвые единицы через 'n' секунд.
//    nowait        = не ждать в конечных точках патрулирования.
//    noslow        = Сохраняйте стандартное поведение юнита (не меняйте его на «безопасный» и «ограниченный»).
//    noai          = Не используйте расширенный ИИ для уклончивых и обходных маневров.
//    showmarker    = Показать маркер области. 
//    trigger       = Покажите сообщение, когда в секторе больше нет единиц.
//    empty:n       = Считайте область пустой, даже если осталось n единиц.
//    track         = Отображение маркера положения и назначения для каждого подразделения.
//
*/ 

if (!isServer) exitWith {};
	
// как далеко должны уйти опфоры, если они находятся под атакой
// установить значение 200-300 при использовании сценария на открытых площадках (в сельской местности)
#define SAFEDIST 75

// насколько близко должен быть объект к цели, чтобы сгенерировать новый
#define CLOSEENOUGH 10

// насколько близко единицы должны быть друг к другу, чтобы делиться информацией
#define SHAREDIST 100

// как долго юниты ИИ должны находиться в режиме тревоги после первоначального обнаружения врага
#define ALERTTIME 60
